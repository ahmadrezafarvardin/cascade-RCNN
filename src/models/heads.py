...
class RPNHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for feature in x.values():
            ...
            bbox_reg_per_level = bbox_reg_per_level.permute(0, 2, 3, 1)  # [N, H, W, A*4]
            bbox_reg_per_level = bbox_reg_per_level.reshape(N, H, W, A, 4)  # [N, H, W, A, 4]
...
class CascadeROIHeads(nn.Module):
    def __init__(self, in_channels, num_stages=3, num_classes=2, stage_loss_weights=None):
        super().__init__()
        ...
        self.stage_iou_thresholds = [0.5, 0.6, 0.7][:num_stages]
    ...
    def forward(self, features, proposals, image_shapes, targets=None, stage=0):
        ...
        if isinstance(proposals, list):
            ...
            for i, props in enumerate(proposals):
                ...
                batch_idx = torch.full((props.shape[0], 1), i, 
        ...
        proposals_normalized = self.normalize_proposals(proposals_concat, feature_map.shape[-2:])
        ...
        if self.training:
            losses = self.compute_loss(class_logits, pred_boxes, proposals, targets, stage)
            return None, losses
        else:
    ...
    def match_proposals_to_targets(self, proposals, targets, iou_threshold):
        ...
        device = proposals[0].device if isinstance(proposals, list) else proposals.device
        ...
        for i in range(num_images):
            ...
            if len(proposals_i) == 0:
                ...
                regression_targets = torch.zeros((0, 4), dtype=torch.float32, device=device)
            elif len(gt_boxes) == 0:
                # No ground truth for this image
                matched_idxs = torch.full((len(proposals_i),), -1, dtype=torch.long, device=device)
                labels = torch.zeros((len(proposals_i),), dtype=torch.long, device=device)
                regression_targets = torch.zeros((len(proposals_i), 4), dtype=torch.float32, device=device)
            else:
                ...
                regression_targets = self.encode_boxes(proposals_i, gt_boxes[matched_idxs])
    ...
    def postprocess_predictions(self, class_logits, pred_boxes, proposals, image_shapes):
        ...
        if isinstance(proposals, list):
            ...
            for i, props in enumerate(proposals):
                ...
                predictions.append({
                    ...
                    "labels": torch.ones(len(boxes_i), dtype=torch.long, device=boxes_i.device)
                })
                start_idx = end_idx
        else:
            ...
            predictions = [{
                ...
                "labels": torch.ones(len(class_probs), dtype=torch.long, device=class_probs.device)
            }]
